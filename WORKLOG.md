...
- 2026-06-22: AdSense/SEO hardening pass. Fixed the `<html lang>` bug by moving the root layout into `app/[locale]/layout.tsx` so `/en` now renders `lang="en"` and `/ko` renders `lang="ko"` (Next.js i18n root-layout-under-dynamic-segment pattern). Removed `app/layout.tsx` and `app/page.tsx`; the `/` to `/ko` redirect now lives in `next.config.ts`. Added a Contact page (`/[locale]/contact`) wired into the header, footer, and sitemap. Added `WebSite` JSON-LD on the home page and `BreadcrumbList` JSON-LD on test and guide detail pages. Added three more guide articles (CPS, typing speed, memory/focus) bringing guides from 5 to 8. Added `.env.example`. Verified with `npm run lint`, `npm run build` (107 static pages), and confirmed the `lang` attribute in the prerendered HTML.
